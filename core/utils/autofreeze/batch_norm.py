    ...
    def forward(ctx, autofreeze_norm: AutoFreezeNorm2d, preserve_rng_state, x, weight, bias):
        ...
        with torch.no_grad():
            ...
            if autofreeze_norm.sparsity_signal: # DAS
                # Handle cases where clip_ratio might be NaN or invalid
                clip_ratio_val = autofreeze_norm.clip_ratio
                if isinstance(clip_ratio_val, torch.Tensor) and torch.isnan(clip_ratio_val):
                    clip_ratio_val = 0.0
                elif isinstance(clip_ratio_val, float) and clip_ratio_val != clip_ratio_val:  # float NaN check
                    clip_ratio_val = 0.0
                    
                clip_strategy = Clipper(clip_ratio_val, mode)
                autofreeze_norm.back_cache_size = int(x.numel() * (1 - float(clip_ratio_val)))
            else:
