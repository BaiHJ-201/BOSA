    ...
    def backward(ctx, grad_out):
        ...
        with torch.random.fork_rng(devices=rng_devices, enabled=ctx.preserve_rng_state):
            ...
            if ctx.n_rm > 0:
                ...
                with torch.no_grad():
                    weight, bias = MyBatchNorm.weight[ctx.remained_idxs], MyBatchNorm.bias[ctx.remained_idxs]
                    if MyBatchNorm.accum_mode == 'exp':
                        running_mean, running_var = running_mean[ctx.remained_idxs], MyBatchNorm.running_var[ctx.remained_idxs]
...
class BN(BaseAdapter):
    ...
    def update_model(self):
        ...
        # entropy_mask = (entropy < 0.05 * math.log(10))
        ...
        # loss = (softmax_entropy(stu_out, ema_out) * 1.0)[entropy_mask].mean()
