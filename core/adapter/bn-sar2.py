...
class BN(BaseAdapter):
    def __init__(self, cfg, model, optimizer):
        ...
        self.margin = 1.0*math.log(cfg.CORRUPTION.NUM_CLASS)
    ...
    def forward_and_adapt(self, batch_data, y):
        ...
        ratio_1_memory = len(filter_ids_1) / memory_size if memory_size > 0 else 0.0
        print(f"filter_ids_1占内存样本比例: {ratio_1_memory:.4f} ({len(filter_ids_1)}/{memory_size})")
        ...
        entropys2 = softmax_entropy(self.model(imgs))
        # entropys2 = entropys2[filter_ids_1]
        filter_ids_2 = torch.where(entropys2 < self.margin)
        ratio_2_memory = len(filter_ids_2) / memory_size if memory_size > 0 else 0.0
        print(f"filter_ids_2占内存样本比例: {ratio_2_memory:.4f} ({len(filter_ids_2)}/{memory_size})")
        # 计算二次损失（仅用二次筛选后的样本）
        loss_second = entropys2[filter_ids_2].mean(0)
        print(f"loss_second（原始）: {loss_second.item():.6f}")
        if self.lambda_bn_w > 0:
            l_soft_alignment = []
            for m in self.model.modules():
                if isinstance(m, MyBatchNorm):
            ...
            l_soft_alignment = torch.tensor(0.0).cuda()
        print(f"l_soft_alignment（第二次）: {l_soft_alignment.item():.6f}")
        loss_second += l_soft_alignment
        print(f"loss_second（含alignment）: {loss_second.item():.6f}")
